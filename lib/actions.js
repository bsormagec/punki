...
const storm = () => async (dispatch, getState) => {
  ...
  dispatch(setShipStocks({
    general: Math.floor(contents.general / 1.2),
    pelts: Math.floor(contents.pelts / 1.2),
    armaments: Math.floor(contents.armaments / 1.2),
    opium: Math.floor(contents.opium / 1.2)
  }));
};
...
export const sail = (islandID) => async (dispatch, getState) => {
  ...
  if (ship.storage.contents.opium > 0 && Math.random() > 0.85) {
