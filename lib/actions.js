...
const offerShipUpgrade = () => async (dispatch, getState) => {
  ...
  const answer = await dispatch(setTravelMessage(
    'A mechanic is offering to upgrade your load capacity by 50kg for 5000$?',
    ['Yes', 'No']
  ));

  if (answer === 'Yes') {
    dispatch(updateCash(cash - 5000));
...
export const inspection = () => async (dispatch) => {
  ...
  await dispatch(setTravelMessage(
    'They have found illegal materials on your vehicle!\nAll of your cargo and cash have been confiscated.',
    ['Oh no.']
  ));
...
const storm = () => async (dispatch, getState) => {
  await wait();
  await dispatch(setTravelMessage('Oh no! A thunder storm!\nYou\'ve lost some of your cargo...'));
  await wait();
...
const discovery = () => async (dispatch, getState) => {
  await wait();
  await dispatch(setTravelMessage('Oh wow! It seems to be your lucky day.\nYou found some cargo...'));
  await wait();
  ...
  dispatch(setShipStocks({
    yarn: Math.floor(Math.random() * 50) + Math.floor(contents.yarn * 1.2),
    parts: Math.floor(Math.random() * 25) + Math.floor(contents.parts * 1.2),
    gems: Math.floor(Math.random() * 10) + Math.floor(contents.gems * 1.2),
    coils: Math.floor(Math.random() * 5) + Math.floor(contents.coils * 1.2),
    tomes: Math.floor(Math.random() * 2) + Math.floor(contents.tomes * 1.2)
  }));
};
...
const raid = () => async (dispatch, getState) => {
  await wait();
  await dispatch(setTravelMessage('Oh no! You are under attack!\nYou\'ve lost some cargo and looters are running...'));
  await wait();
