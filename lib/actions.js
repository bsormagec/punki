...
const offerShipUpgrade = () => async (dispatch, getState) => {
  ...
  const answer = await dispatch(setTravelMessage(
    'A ship builder offers to upgrade your ship. Upgrade your storage by 100kg for 1000$ ?',
    ['Yes', 'No']
  ));
...
const storm = () => async (dispatch, getState) => {
  ...
  dispatch(setShipStocks({
    general: Math.floor(contents.general / 2),
    pelts: Math.floor(contents.pelts / 2),
    armaments: Math.floor(contents.armaments / 2),
    opium: Math.floor(contents.opium / 2)
  }));
};
